    ...
    <div class="preview-2">
        <div id="nivoslider" class="slides">
            @foreach($banners as $banner)
                <a href="#">
                    <img src="/img/banner/{{ $banner->imagem_fundo }}" alt="{{ $banner->titulo }}" title="#slider-{{ $loop->index }}-caption"/>
                </a>
            @endforeach
        </div>
        @foreach($banners as $banner)
            <div id="slider-{{$loop->index}}-caption" class="nivo-html-caption nivo-caption">
                @if($banner->imagem_destaque)
                    <div class="layer-1">
                        <img src="/img/banner/{{ $banner->imagem_destaque }}" alt="{{ $banner->titulo }}"/>
                    </div>
                @endif
                <div class="banner-content slider-{{$loop->index}}">
